<template>
  <div class="space-y-4">
    <h1 class="text-2xl font-bold">Avans Onayları</h1>
    ...
    <div v-if="requests.length > 0" class="bg-white shadow rounded-lg overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="request in requests" :key="request._id">
            ...
            <td class="px-6 py-4 whitespace-nowrap">{{ request.amount.toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' }) }}</td>
            <td class="px-6 py-4 whitespace-nowrap">{{ formatDate(request.requestDate) }}</td>
     ...
     <div v-else-if="!loading" class="text-center text-gray-500">
      Bekleyen avans talebi bulunmuyor.
    </div>
...
const fetchRequests = async () => {
  try {
    loading.value = true;
    const response = await api.get('/advancerequests');
...
const updateStatus = async (id, status, reason = '') => {
  try {
    ...
    await api.put(`/advancerequests/${id}/status`, payload);
